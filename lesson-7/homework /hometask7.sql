...
Select COUNT(CustomerId) as Numburofrows from Customers

--4.Write a query to count the number of unique product categories from the Products table.

Select COUNT(distinct Category) as uniqproduct  from Products
...
Select AVG(age) as EmpAge from Employees
...
Select CustomerId,
Count(CustomerID) as [totalsales]
from Customers
group by CustomerID;
...
Select SaleID,
       sum(SaleAmount) as totalsum,
	   avg(saleAmount) as totalavg
from Sales
group by SaleID;

--12.Write a query to count the number of employees from the Department HR.

Select COUNT(EmployeeID) as numberHR
from Employees
